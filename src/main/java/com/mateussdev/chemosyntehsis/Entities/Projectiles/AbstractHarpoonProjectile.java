...
public abstract class AbstractHarpoonProjectile extends AbstractArrow {
    ...
    public void DragTargetToParent(double strength) {
        ...
        Entity target = getTarget();
        if(parent != null && target != null) {
            if(target instanceof BaseOrganelle LE) {
                RetractHarpoon();
                return;
            }
            Vec3 force = parent.position().subtract(target.position()).normalize().scale(strength);
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if(target instanceof AbstractArrow) return;
...
//                        if(getOwner().distanceTo(getTarget()) < 1) {
//                            RetractHarpoon();
//                        }
                    }
                        ...
                        if(dragItself) {
                            ...
                            if(Mth.sqrt((float) getOwner().distanceToSqr(getHarpoonedPos())) < 1) {
                                RetractHarpoon();
                            }
                        } else {
                            attachedToBlockTick();
    ...
    public void RetractHarpoon() {
        ...
        if((parent != null && this.distanceTo(getOwner()) > 2) && getHarpoonLeftLength() > -200) {
            ...
            this.setDeltaMovement(retractDirection.scale(strength*3));
        }
        else {
            discard();
        }
