...
public class MutatedHarpoonEntity_Render extends GeoEntityRenderer<MutatedHarpoonEntity> {

    private final StemModel stemModel;

    public MutatedHarpoonEntity_Render(EntityRendererProvider.Context context) {
        super(context, new MutatedHarpoonEntity_Model());
        this.stemModel = new StemModel();
    }

    @Override
    public void preRender(PoseStack poseStack, MutatedHarpoonEntity animatable, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        if(!isReRender) {
    ...
    @Override
    public boolean shouldRender(MutatedHarpoonEntity pLivingEntity, Frustum pCamera, double pCamX, double pCamY, double pCamZ) {
        return true;
    }

    //TODO:Make the renderer different
//    @Override
//    public void actuallyRender(PoseStack poseStack, MutatedHarpoonEntity animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
...
//
//        Vec3 parentPos = animatable.getOwner().getEyePosition(partialTick);
